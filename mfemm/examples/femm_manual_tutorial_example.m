...
% fmesher is crashing, using .exe file directly
system(['C:\Dados\xfemm_mingw_win64\cfemm\bin\fmesher ' filename]);
...
%     myfpproc.plotBfield(-1, -4.5, 6, 9, 'Method', 0);

    myfpproc.plotBfield(-1, -4.5, 6, 9, 'Method', 1, 'Points', 500);

    % The vector potential can also be plotted
%     myfpproc.plotAfield(-1, -4.5, 6, 9, 'Points', 500);
    myfpproc.plotAfield(-1, -4.5, 6, 9, 'Method', 0, 'Points', 500);
end
...
elm = myfpproc.getelements;
ver = myfpproc.getvertices;

pile_elem = [elm(:,1); elm(:,2); elm(:,3)];
pile_ver = [ver(:,1:2); ver(:,3:4); ver(:,5:6)];

[uElem, iE, iUe] = unique(pile_elem);

pile = [uElem pile_ver(iE,:)];

dtri = delaunay(pile(:,2),pile(:,3));

datafcn = myfpproc.geta(pile(:,2),pile(:,3));

% trisurf(dtri,pile(:,2),pile(:,3),datafcn,'LineStyle','none')
trisurf(dtri,pile(:,2),pile(:,3),datafcn,'LineStyle','none','FaceAlpha','1')

% pcolor(dtri, pile(:,2),pile(:,3),datafcn)
% patch('Faces',dtri,'Vertices',[pile(:,2),pile(:,3)],'FaceVertexCData',datafcn.','FaceColor','flat');
